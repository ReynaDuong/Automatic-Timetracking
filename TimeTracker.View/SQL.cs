...
namespace TimeTracker.View
{
	public class Sql
	{
		...
		public static List<Association> LoadAssociations(int type, string projectId)
		{
			...
			try
			{
				while (reader.Read())
				{
					...
					r.taskName =
						Global.allTaskIdName
							[r.taskId]; //lookup task name in global dictionary, which is defined before this routine is called
		...
		public static void InsertRule(int type, string value, string workspaceId, string projectId, string taskId,
			string workspaceName, string projectName, string taskName)
		{
			var query = string.Empty;
			...
			if (type == 1)
			{
				...
				{
					query = "INSERT INTO `Processes` (`Name`, `TaskID`, `projectId`) VALUES('" + value + "', '" +
					        taskId + "', '" + projectId + "')";
				}
				else
				{
			...
			else if (type == 2)
			{
				...
				{
					query = "INSERT INTO `URLs` (`URL`, `TaskID`, `projectId`) VALUES('" + value + "', '" + taskId +
					        "', '" + projectId + "')";
				}
				else
				{
		...
		public static string QueryTaskId(int type, string value, string projectId)
		{
			...
			if (type == 1)
			{
				query = "SELECT Processes.TaskID FROM Processes WHERE Processes.Name = '" + value +
				        "' AND Processes.ProjectID = '" + projectId + "'";
			}
			else if (type == 2)
			{
				query = "SELECT URLs.TaskID FROM URLs WHERE URLs.URL = '" + value + "' AND URLs.ProjectID = '" +
				        projectId + "'";
			}
		...
		public static string QueryTaskName(string taskId, string projectId)
		{
			...
			var query = "SELECT Tasks.Name FROM Tasks WHERE Tasks.ID = '" + taskId + "' AND Tasks.ProjectID = '" +
			            projectId + "'";
		...
		public static bool IfExist(int type, string projectId, string value)
		{
			...
			{
				query = "SELECT EXISTS(SELECT * FROM Processes WHERE Name = '" + value + "' AND projectId = '" +
				        projectId + "') as `is -exists`";
			}
			else if (type == 5)
			{
				query = "SELECT EXISTS(SELECT * FROM URLs WHERE URL = '" + value + "' AND projectId = '" + projectId +
				        "') as `is -exists`";
			}
